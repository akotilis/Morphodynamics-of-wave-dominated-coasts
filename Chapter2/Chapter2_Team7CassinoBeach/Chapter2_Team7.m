...
for ii = 1:length(low_tide(1,:))
    
    [S f edf conf95Interval] = VarianceDensitySpectrum(low_tide(:,ii),array_blocks(3),Fs);
...
%% Relation between Hm0 and H1/3

%Using a referenence height array for the fit
H_ref = [0 0.25 0.5 0.75 1 1.25 1.5 1.75 2, 2.25, 2.5];

%Use polyfit to determine the linear relation between Hm0 and H1/3
s = polyfit(Hm0, H13_total, 1);
slope = s(1);
intercept = s(2);
...
figure()
plot(Hm0(:,1),H13_total(:,1), 'ro', 'linewidth', 1)
hold on
plot(Hm0(:,2),H13_total(:,2), 'bo', 'linewidth', 1)
hold on
plot(Hm0(:,3),H13_total(:,3), 'go', 'linewidth', 1)
hold on
plot(H_ref, fit, 'k-')
legend('Low tide','Mid tide', 'High tide', 'Linear fit')
xlabel('H_{1/3} [m]')
ylabel('H_{m0} [m]')
title('Linear relation between H_{m0} and H_{1/3}')
