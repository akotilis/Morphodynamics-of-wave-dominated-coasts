...
for ii = 1:length(array_names) 
    ...
    Ur_obs(jj,ii) = ursell_number(wavenumber,h(jj,ii),Hrms0(jj,ii)); 
...
hold on;
scatter(Ur_obs(:,1),skewness(:,1),'*'); 
scatter(Ur_obs(:,2),skewness(:,2),'*'); 
scatter(Ur_obs(:,3),skewness(:,3),'*'); 
...
hold on
scatter(Ur_obs(:,1),assymetry(:,1),'*'); 
scatter(Ur_obs(:,2),assymetry(:,2),'*'); 
scatter(Ur_obs(:,3),assymetry(:,3),'*'); 
title('Asymmetry as a function of the Ursell number')
xlabel('Ursell number (log scale)')
ylabel('Asymmetry') 
legend('Asymmetry Ruessink', 'Asymmetry low tide', 'Asymmetry mid tide', 'Asymmetry high tide'); 
grid on 
...
%     
%     [skewness(i) assymetry(i)] = skewness_asymmetry(wave_data(1,i));
%     
%     phi(i) = computation_phi(skewness(i), assymetry(i));
%     
%     r(i) = computation_r(skewness(i), assymetry(i));
%     
%     Uw = velocity_amplitude(waves.ht, waves.Hrms, T0(1));
%     
%     [u(:,i) t(:,i)] = waveshape(r(i), phi(i), Uw, T0(1));
%     
% end 
